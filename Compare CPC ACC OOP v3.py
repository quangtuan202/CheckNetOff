...
class Dataframe:
    ...
    def new_key(self):
        ...
        self.dataframe[key+'_dot']=self.dataframe[key].apply(self.policy_dot)

        # Create a columns for key with '-' removed
        self.dataframe[key+'_hyphen']=self.dataframe[key].str.split('-').str[0]

        # Create a columns for key with 'EN' removed
        self.dataframe[key+'_en']=self.dataframe[key].str.split('EN').str[0]

        # Create a columns for key contains character to dept
        self.dataframe[key+'_dept']=self.dataframe[key].apply(self.policy_dept)
    #-------------------------------------------------------------------------------------------        
...
class Dataframe_acc(Dataframe):
    def __init__(self,file_name,use_cols,original_cols,new_cols,sum_cols,account_list,account_type,cols_of_accounts,final_cols,key):
        ...
        self.acc_combined_list=combine_list(self.account_list)
...
class list_of_unique_item():
    def __init__(self,list_a,list_b,key):
        ...
        self.df_unique=pd.DataFrame(self.df,columns=[key])
