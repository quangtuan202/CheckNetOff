...
def remove_fx_duplicate(tbl=pd.DataFrame()):
    ...
    for i in range(0,len(col_1)):
        if i<1:
            col_1_new.append(col_1[0])
            col_2_new.append(col_2[0])
            col_3_new.append(col_3[0])
...
def button_run_click():
        ...
        if premium_or_claim == 'premium':
            if file_or_folder=='folder':
                ...
                for i in range(len(df)):
                    if df[i].shape[1] in [17, 18, 19, 20,21,22]:
            ...
            if line_or_text_method=='line':
                ...
                df_concat_1[['Premium', 'RI Premium', 'RI Comm', 'RI Tax', 'Net RI Premium']]=df_concat_1[['Premium', 'RI Premium', 'RI Comm', 'RI Tax', 'Net RI Premium']].replace(' ', '', regex=True)#.astype('float64')
            else:
            ...
            if line_or_text_method=='line':
                df_concat_1 = df_concat.loc[(df_concat['No']!='No') & (df_concat['No'].notnull())& (df_concat['No']!='')]
                df_concat_1[['Total Loss Amount', 'Survey Fee', 'Total Amount', 'RI Amount']]=df_concat_1[['Total Loss Amount', 'Survey Fee', 'Total Amount', 'RI Amount']].replace(' ', '', regex=True)#.astype('float64')
            else:
    ...
    if premium_or_claim == 'claim':
        df_concat_1[['Total Loss Amount', 'Survey Fee', 'Total Amount', 'RI Amount']]=df_concat_1[['Total Loss Amount', 'Survey Fee', 'Total Amount', 'RI Amount']].astype('float64')     
    else:       
        #df_concat_1[['Premium', 'RI Premium', 'RI Comm', 'RI Tax', 'Net RI Premium']]=df_concat_1[['Premium', 'RI Premium', 'RI Comm', 'RI Tax', 'Net RI Premium']].replace(' ', '', regex=True).replace('','0',regex=True)
        df_concat_1[['Premium', 'RI Premium', 'RI Comm', 'RI Tax', 'Net RI Premium']]=df_concat_1[['Premium', 'RI Premium', 'RI Comm', 'RI Tax', 'Net RI Premium']].astype('float64')
    ...
    writer = ExcelWriter(f"{saving_folder_path}/{saving_file_name.get()}.xlsx",engine='xlsxwriter')
    # Get the xlsxwriter workbook and worksheet objects.
    #workbook  = writer.book
    df_concat_final.to_excel(writer, sheet_name='BANG_KE')
    
    fx_table.to_excel(writer, sheet_name='TY_GIA')
    #bang_ke=writer.sheets['BANG_KE']
    # Add some cell formats.
    #number_format = workbook.add_format({'num_format': '#,##0.00'})
    # Set the format but not the column width.
    #for col in ('G','I','J','K','L','S'):
     #   bang_ke.set_column(col+':'+col, None, workbook.add_format({'num_format': '0.00'})) 
    writer.save()
