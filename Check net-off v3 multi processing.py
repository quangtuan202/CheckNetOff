...
def process_single_df(df):
    ...
    df['Check']=df.groupby(['PolicyForChecking'],sort=False)[['Chênh lệch VND']].transform(netOffCheckNonMarine,df=df,col=colNonMarine,totalLimit=10,combinationLimit=10000000)
    return df
...
if __name__ == '__main__':
    ...
    df['PolicyForChecking']=df.apply(lambda x: right_find(x['Số đơn'],x['Phí bảo hiểm VND CPC'],x['Phí bảo hiểm VND ACC']),axis=1).astype(str)
